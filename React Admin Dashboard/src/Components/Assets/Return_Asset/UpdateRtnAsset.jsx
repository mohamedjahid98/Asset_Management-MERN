  ...
  useEffect(() => {
    axios.get("http://localhost:3001/issues/getIssuedata/" + id)
      .then(result => {
        ...
        setEmpsname(result.data.empsname)
        setAsset_type(result.data.asset_type)
        const formattedDob = new Date(result.data.ret_date).toISOString().split('T')[0];
        setRet_date(formattedDob)

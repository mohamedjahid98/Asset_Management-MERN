                ...
                filteredAssets.map((item) => {
                  return (
                    <tr key={item._id}>
                      ...
                      <td>{item.ret_date}</td>
                      <td style={{ color:item.reason.toLowerCase() === 'success' ? 'green' :item.reason.toLowerCase() === 'repair' ? 'blue' :'red', fontSize: '16px',  // Adjust the font size as needed
  fontWeight: 'bold'}}>{item.reason}</td>

                      <td><Link to={`/return-asset/update/${item._id}`}>
